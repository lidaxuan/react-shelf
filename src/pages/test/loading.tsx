 ...
 * @Description: 
 * @Author: 李大玄
 * @Date: 2021-07-15 15:55:20
 * @FilePath: /react-shelf/src/pages/test/loading.tsx
 */
...
function fetchData<T>(query: any): Promise<T> {
  const obj = {
    data: [],
    total: 10,
  } as any;
...
export default class LiveAdd extends React.Component<any, any> {
  ...
  async getOrderList(query?: Query) {
    ...
    const result = await fetchData<any>(param);
  ...
  render(): React.ReactElement {
    ...
    return (
      <div className="pd20">
        ...
        <Form name="basic" className="createBox" initialValues={this.state.form} onFinish={this.onFinish} {...layout} >
          <Form.Item label="主播姓名" name="order_no" >
            <Input placeholder="请输入主播姓名" className="circleBorder" />
          </Form.Item>
          <Form.Item label="主播昵称" name="order_no" >
            <Input placeholder="请输入主播昵称" className="circleBorder" />
          </Form.Item>
          <Form.Item label="手机号" name="order_no">
            <Input placeholder="请输入手机号" className="circleBorder" />
          </Form.Item>
          <Form.Item label="微信号" name="order_no" >
