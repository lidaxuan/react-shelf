...
export default abstract class Goods<Props extends GoodsProps, State extends Interface.Goods> extends React.Component<Props, State> {
  ...
  // 数据转换
  transformData(data: State): State {
  ...
  getUploadFileNode(label: string, key: string, index: number, remove?: boolean): React.ReactNode {
    const files: Array<string> = this.state[key];
    if (files && files.length) {
      const name = `${key}[${index}]`;
      const onRemove = () => {
        files[index] = void 0; // 设置为空
        const data = {};
        data[key] = files;
        this.setFieldValue(name, void 0); // 设置 antd form 中对应的数据
        this.setState(data);
      };
      const data = {
        type: 'goods'
      ...
      return (<Form.Item name={ name } label={ label }>
        <Upload accept="image/*" data={ data } remove={ remove } onRemove={ onRemove }></Upload>
      </Form.Item>);
    }
    return void 0;
  }
  ...
  // 商品缩略图
  getGoodsHumbnailList () {
    const name = 'humbnail_img';
    let key = 0;
    
    const temp = _.map(this.state[name], (src: string) => {
      if (_.isUndefined(src)) {
        return void 0;
      }
      return true;
    });
    const tempListLength = _.size(_.compact(temp));

    return (<div className="flex">
      {
        _.map(this.state[name], (src: string, index: number) => {
          // 为 undefined 时表示该图片被删除了
          if (_.isUndefined(src)) {
            return void 0;
          }
          key += 1;
          const label = key === 1 ? '商品缩略图' : void 0;
          return (<div key={ index }>
            { this.getUploadFileNode(label, name, index, tempListLength > 1)}
          </div>);
        })
      }
    </div>);
  ...
  // 商品详情图
  getGoodsDetailList (): React.ReactNode {
    const name = 'detail_img';
    let key = 0;
    ...
    return (<div className="flex">
      {
        _.map(this.state[name], (src: string, index: number) => {
          ...
          key += 1;
          const label = key === 1 ? '商品详情图' : void 0;
          return (<div key={ index }>
            { this.getUploadFileNode(label, name, index, tempListLength > 1)}
          </div>);
        })
